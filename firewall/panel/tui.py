...
class SessionPanel(Static):
    ...
    mode_badge: reactive[str] = reactive("[yellow]● BALANCED[/yellow]")
    started: float = 0.0
...
class FirewallPanel(App):
    ...
    def on_mount(self) -> None:
        ...
        # cheap config re-read so the subtitle updates after `firewall config mode ...`
        self.set_interval(2.0, self._refresh_subtitle)

    _mode_badge = {
        "strict":   "[red]● STRICT[/red] (block known-bad + unknown)",
        "balanced": "[yellow]● BALANCED[/yellow] (block known-bad, short-circuit unknown)",
        "observe":  "[green]● OBSERVE[/green] (record only, never block)",
    }

    def _refresh_subtitle(self) -> None:
        # re-read mode from disk so this stays in sync with `firewall config mode`
        mode = "balanced"
        if self.cfg is not None:
            try:
                from firewall.config import load_config, config_path
                self.cfg = load_config(config_path())
            except Exception:
                pass
            mode = getattr(getattr(self.cfg, "policy", None), "mode", "balanced")
        model = self.cfg.driver.model if self.cfg else "—"
        badge = self._mode_badge.get(mode, mode)
        # Textual sub_title supports markup; falls back gracefully on plain Header
        self.sub_title = f"driver: {model}   policy: {mode}"
        # also surface the enforcement state to the session card
        try:
